...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        new_sentences = self.update_new_sentences()
        while new_sentences:
    ...
    def update_safes_mines(self):
        # repeat update if an update was made
        counter = 1
        while counter:
            counter = 0
            for sentence in self.knowledge:
    ...
    def update_new_sentences(self):
        ...
        for set1 in self.knowledge:
            ...
            if not set1.cells:
                remove_sentences.append(set1)
                continue

            for set2 in self.knowledge:
                # all cells removed from the sentence
                if not set2.cells:
                    remove_sentences.append(set2)
                    continue

                if set1 != set2:
                    if set2.cells.issubset(set1.cells):
                        cells_difference = set1.cells.difference(set2.cells)
                        count_difference = set1.count - set2.count

                        new_subset = Sentence(cells_difference, count_difference)
                        if new_subset not in self.knowledge:
                            new_sentences.append(new_subset)
