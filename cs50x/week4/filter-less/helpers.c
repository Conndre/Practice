...
void grayscale(int height, int width, RGBTRIPLE image[height][width])
{
    ...
    for (int i = 0; i < height; i++)
    {
        for (int j = 0; j < width; j++)
        {
            ...
            int average = round(((float) image[i][j].rgbtRed + (float) image[i][j].rgbtGreen +
        ...
        for (int j = 0; j < width; j++)
        {
            ...
            for (int k = i - 1; k <= i + 1; k++)
            {
                for (int m = j - 1; m <= j + 1; m++)
                {
                    ...
                    {
                        totalRed += copy[k][m].rgbtRed;
                        totalGreen += copy[k][m].rgbtGreen;
                        totalBlue += copy[k][m].rgbtBlue;
                        count++;
                    }
                }
            }
